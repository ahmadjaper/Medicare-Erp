      ...
      {showUserRole && (
        <div className="d-flex align-items-center gap-2">
          <div className="navbar-user-text d-none d-md-block text-start me-1">
            <div className="navbar-user-name" style={{ fontSize: '0.8rem', fontWeight: 700 }}>{userProfile.fullName}</div>
            <div className="navbar-user-role" style={{ fontSize: '0.7rem', color: 'var(--text-muted)' }}>{userProfile.jobTitle}</div>
          </div>
          <select 
            value={currentRole} 
        ...
        {showProfileMenu && (
          <div className="dropdown-menu dropdown-menu-end show shadow-sm" style={{position: 'absolute', right: 0, top: '100%', minWidth: '200px', zIndex: 1050}}>
            <div className="px-3 py-2 border-bottom">
              <div className="fw-bold">{userProfile.fullName}</div>
              <div className="text-muted small">{userProfile.jobTitle}</div>
            </div>
            <button className="dropdown-item d-flex align-items-center gap-2 py-2" onClick={() => { setShowProfileMenu(false); navigate('/settings/profile'); }}>
